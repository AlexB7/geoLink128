...
TXT_AREA	=	182	;top of text input area
...
@50:	LoadW	r0,title
	...
	LoadW	rightMargin,636	;is still set from the login screen
	jsr	PutString
	PopW	rightMargin
	rts
...
; -----------------------------------------------------------
cntBold:
	lda #88		;max chars across screen.  40-column mode uses 63, 80-column uses 126 with VIP, 88 with BSW 128
	sta	maxChars
	ldx	#0
	stx	boldon
	ldy	#0
...
@40:
	iny
	cpy	#88	;max chars across screen.  40-column mode uses 63, 80-column uses 126 with VIP, 88 with BSW 128
	bne	@10
...
putMsg:	jsr	savTxtSt	;save text status
	LoadW	leftMargin,0
	LoadW	rightMargin,636		; 40 column is 318
...
; VDC scroll only..
; -----------------------------------------------------------
SCR_FROM80	=	1920	;scroll from - 640 * 3
SCR_TO80	=	1280	;scroll to - 640 * 2
SCR_AMT80	=	160	;scroll amount in scan lines

scroll:
	jsr TempHideMouse
	
	; Save value of register 24
	lda #24
	jsr vdcr_write
	MoveB vdcd, a7L
...
@40:	lda	keyTemp
	jsr	SmallPutChar
	AddVW	7,stringX	;advance cursor - 5 for VIP font, 7 for BSW 128
@50:	clc
	rts
;	------------------------------------------------
insert:	ldy	inputNdx
...
backUp:	lda	r11L
	sec
	sbc	#7			; 5 for VIP font, 7 for BSW 128
	sta	r11L
	lda	r11H
	sbc	#0
	sta	r11H
...
sndIcons:	.byte	1	;one icon
	.word	300	; x position
	.byte	193 ; y position - 192 for 40 column, 193 for 80 column - no effect?
;	------------------------------------------------
	.word	sendIcon
	.byte	72,TXT_AREA+3	;x in cards, y in pixels - 35 for 40 column, 72 for 80 column
